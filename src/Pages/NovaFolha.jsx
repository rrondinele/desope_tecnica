  ...
  const handleNext = () => {
    ...
    if (currentStep === 1) {
      ...
      const req = [
        ...
        ['caracteristica', 'Característica'], // a Adicionar esta linha
      ...
      if (!numeroFmRaw.startsWith("FM -")) {
        ...
        alert("O nAmero da Folha de Medição deve começar com 'FM -'.");
      ...
      if (possuiVersao && !contextoCorrecaoValido) {
        ...
        alert("O sufixo '-vX' s? pode ser usado em folhas reprovadas que est??o em corre???o.");
      ...
      if (!/^\d{2}\.\d{3}$/.test(numeroBase)) {
        ...
        alert("O n?mero da Folha de Medi???o deve seguir o formato 'FM - XX.XXX' (ou 'FM - XX.XXX-vX' para corre???es).");
      ...
      if (expectedPrefix && !numeroBase.startsWith(expectedPrefix)) {
        ...
        alert(
          ...
          `Para a base ${regionalKey} em ${formData.tipo_processo}, o n?mero deve come??ar com '${expectedPrefix}' (exemplo: FM - ${expectedPrefix}123).`
    ...
    try {
      ...
      if (cleanedPayload.id) {
        ...
        if (isEditing) {
          ...
          alert("Nao e possível criar uma nova folha enquanto estiver editando uma existente. Volte a lista e tente novamente.");
